...
def create_default_style(path):
    """Génère le style.qss par défaut si l'utilisateur l'a supprimé (Fresh Install)"""
    os.makedirs("Data", exist_ok=True)
    default_qss = """
QMainWindow, QWidget { background-color: #09090b; color: #e4e4e7; font-family: "Inter", "Segoe UI", "-apple-system", "Helvetica Neue", sans-serif; font-size: 14px; }
...
QLineEdit:focus, QComboBox:focus, QSpinBox:focus { border: 1px solid #FF1796; background-color: #202024; }
...
QPushButton:pressed { background-color: #18181b; border: 1px solid #FF1796; color: #FF1796; }
QListWidget, QTextEdit, QScrollArea { background-color: #09090b; border: 1px solid #27272a; border-radius: 10px; padding: 6px; outline: none; color: #ffffff; font-size: 15px; font-weight: 600; }
...
QListWidget::item:selected { background-color: #1e293b; border: 1px solid #FF1796; color: #ffffff; }
...
QPushButton#NavButton[active="true"] { background-color: #FF1796; border: none; font-weight: bold; }
...
"""
    with open(path, "w", encoding="utf-8") as f:
        f.write(default_qss.strip())

def main():
    app = QApplication(sys.argv)
    app.setStyle("Fusion")

    style_path = os.path.join("Data", "style.qss")
    
    # 🟢 Auto-génération si manquant
    if not os.path.exists(style_path):
        create_default_style(style_path)
    ...
    if os.path.exists(style_path):
        with open(style_path, "r", encoding="utf-8") as f:
            ...
            import re
            variables = re.findall(r"(@[\w-]+)\s*:\s*(#[\w\d]+|rgba?\([\d\s,.]+\));", style_content)
            for var_name, var_value in variables:
                style_content = style_content.replace(var_name, var_value)
